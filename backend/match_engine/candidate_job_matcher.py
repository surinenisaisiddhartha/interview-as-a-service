...
class CandidateJobMatcher:
    ...
    # ── MANUAL: match via JSON (file paths or dicts) ─────────────────────────────
    def match_from_json(
        self, resume_data: Union[dict, str], jd_data: Union[dict, str]
    ) -> dict:
        """
        Accepts raw JSON dicts (or file paths). Looks up candidate by email and
        job by title in PostgreSQL, then saves match to matches table.
        ...
        if candidate and job:
            ...
            try:
                match_result = self._calculate_scores(candidate, job)
                self._save_match(session, candidate, job, match_result)
                session.commit()
                return match_result
            finally:
                session.close()
    ...
    def _calculate_scores(self, candidate: Candidate, job: Job) -> dict:
        ...
        if isinstance(candidate.skills, dict):
            for skills_in_category in candidate.skills.values():
                if isinstance(skills_in_category, list):
                    candidate_skills_raw.extend(skills_in_category)
    ...
    def _save_match(self, db, candidate: Candidate, job: Job, match_result: dict):
        ...
        if existing:
            existing.required_skills_score = match_scores["required_skills_score"]
            existing.preferred_skills_score = match_scores["preferred_skills_score"]
            existing.experience_score = match_scores["experience_score"]
            existing.final_match_percentage = match_scores["final_match_percentage"]
            ...
            match_row = Match(
                ...
                required_skills_score=match_scores["required_skills_score"],
                preferred_skills_score=match_scores["preferred_skills_score"],
                experience_score=match_scores["experience_score"],
                final_match_percentage=match_scores["final_match_percentage"],
    ...
    def _score_from_json(self, candidate_data: dict, job_data: dict) -> dict:
        ...
        JsonFileSaver.save_json(result, self.output_dir, "match_result")
        return result
